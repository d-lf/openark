...
A VTXO is a logical output representing claim to value under a spending condition:

- Bound to a parent VTXO or root
- Spent by cosigned transitions
- Redeemable on-chain via unilateral exit

VTXOs form a directed acyclic graph (tree).

---

## 7. Round Lifecycle

RM: We should have a state diagram here, clearly defining the round lifecycle in the states Initiated, Started, Closed, and Recycled.

Rounds proceed as defined in the examples:

1. `new_round_initiate`
2. `new_round_join`
3. `new_round_vtxo_tree_proposal`
4. `new_round_vtxo_tree_accept`
5. `new_round_prepare_start`
6. `new_round_start`
7. `round_closed`
8. `round_recycled`

Rounds are bounded by a predetermined Bitcoin block height. Two values are important here, the closing block and the recycle block. 
The closing block is the block that initiates the closing state. The recycle block is the block that initiates the recycle state, where the ASP can unilateraly sign the recycle transaction.   
